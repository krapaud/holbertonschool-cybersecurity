...
| Item | Current State | Target State | Gap | Risk Level |
| --- | --- | --- | --- | --- |
| SSH exposure | SSH exposed to the entire internet, no VPN, root login enabled. | SSH accessible only through VPN, root login disabled. | No VPN exists, SSH is reachable directly from the internet. | Critical |
| Root authentication | Root login is enabled, anyone with credentials can connect as root | Root login disabled, users must connect with personal accounts and escalate with sudo | Root login not disabled, no sudo-based access control in place. | Critical |
